...
def register_export_tools(mcp: FastMCP, client: AsepriteClient, config: Config) -> None:
    ...
    @mcp.tool()
    async def export_sprite(sprite_path: str, output_path: str, format: str, frame_number: int = 0) -> str:
        ...
        if format_lower not in valid_formats:
            return json.dumps({"error": f"invalid format: {format} (valid: png, gif, jpg, bmp)"})
        ...
        if "Exported successfully" not in output:
    ...
    @mcp.tool()
    async def export_spritesheet(sprite_path: str, output_path: str, layout: str = "horizontal", padding: int = 0, include_json: bool = False) -> str:
        ...
        if layout not in valid_layouts:
            return json.dumps({"error": f"invalid layout: {layout} (valid: horizontal, vertical, rows, columns, packed)"})
        ...
        try:
            ...
            return json.dumps({"error": "Failed to parse spritesheet output", "details": output})
        
        return json.dumps(parsed)
    
    @mcp.tool()
    async def import_image(sprite_path: str, image_path: str, layer_name: str, frame_number: int, x: int = None, y: int = None) -> str:
        ...
        if "Image imported successfully" not in output:
    ...
    async def save_as(sprite_path: str, output_path: str) -> str:
        ...
        if not (output_path.endswith(".aseprite") or output_path.endswith(".ase")):
            return json.dumps({"error": "output_path must have .aseprite or .ase extension"})
        ...
        try:
            ...
            return json.dumps({"error": "Failed to parse save_as output", "details": output})
